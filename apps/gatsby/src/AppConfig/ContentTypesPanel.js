  ...
  return (
    <>
      ...
      {fullEnabledTypes.map(({ sys }, index) => {
        return (
          <Flex marginBottom="spacingM" key={`flex-${sys.id}`}>
          ...
          <Flex marginRight="spacingS">
            ...
            >
              ...
              {sortedContentTypes.map(({ name, sys }) => {
                ...
                const selected =
                  fullEnabledTypes.findIndex((type) => type.name === name) !==
                  -1;
                return (
                  <Option
                    disabled={selected}
...
const ContentTypesPanel = ({
  ...
  <Typography>
    ...
    <Paragraph>Optionally, define slugs using:</Paragraph>
    <List>
      <ListItem key={`instruction-1`}>
        <Paragraph>
          Strings (must be contained in quotes, will be the same for every
          entry): <strong>"resources"</strong>
        </Paragraph>
      </ListItem>
