...
const getMockContent = () => ({
  ...
  publishedAt: "2021-08-23T15:27:27.861Z",
});
...
describe("Gatsby App Sidebar", () => {
  ...
  it("should not execute callWebhook", () => {
    jest.mock("./utils.js", () => ({
      callWebhook: jest.fn(),
    }));
    const component = renderer.create(<Sidebar sdk={mockSdk} />);
    const instance = component.getInstance();
    const spyCallWebHook = jest.spyOn(utils, "callWebhook");
    instance.onSysChanged(getMockContent());
    expect(spyCallWebHook).toHaveBeenCalledTimes(0);
    instance.setState({ lastPublishedDateTime: "2021-08-23T15:27:27.861Z" });
    instance.onSysChanged(getMockContent());
    expect(spyCallWebHook).toHaveBeenCalledTimes(0);
  });

  it("should execute callWebhook", () => {
    jest.mock("./utils.js", () => ({
      callWebhook: jest.fn(),
    }));
    const component = renderer.create(<Sidebar sdk={mockSdk} />);
    const instance = component.getInstance();
    const spyCallWebHook = jest.spyOn(utils, "callWebhook");
    instance.setState({ lastPublishedDateTime: "2021-09-23T15:27:27.861Z" });
    instance.onSysChanged(getMockContent());
    expect(spyCallWebHook).toHaveBeenCalledTimes(1);
  });
});
